...
class Calendar {
  constructor(month, year) {
    this.month =
      isNaN(month) || month == null ? cal_current_date.getMonth() : month;
    this.year =
      isNaN(year) || year == null ? cal_current_date.getFullYear() : year;
    this.html = "";
    ...
    for (let i = 0; i < 9; i++) {
      for (let j = 0; j <= 6; j++) {
        new_POSTS.map(postDay => {
          if (
            postDay.date.time[0] === this.month &&
            postDay.date.time[1] === day
          ) {
  ...
  renderCalendar(firstDay, startingDay, monthLength) {
    ...
    html +=
      '<button id="left-btn" class="arrow_btn"><</button> ' +
      "<span class='monthName'>" +
      monthName +
      " " +
      this.year +
      "</span>" +
      '<button class="arrow_btn" id="right-btn">></button>';
    html += "</th></tr>";
  ...
  generateHTML() {
    ...
    calendarDiv.innerHTML = this.renderCalendar(
      firstDay,
      startingDay,
      monthLength
    );
...
function createCal(month, year) {
  ...
  rightBtn.addEventListener("click", function() {
    if (currentPosition < arrOfDatesToCal.length - 1) currentPosition++;
